...
impl Builder {
    ...
    /// Apply the peer's authenticated ALPS MAX_FIELD_SECTION_SIZE before requests.
    ///
    /// This narrow hook does not negotiate ALPS or process other settings. Call
    /// only after verifying the TLS handshake and validating the ALPS payload.
    /// Control-stream SETTINGS are still required and must not reduce this limit.
