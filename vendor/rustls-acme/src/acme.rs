...
use crate::config::ExternalAccount;
...
use crate::jose::{external_account_binding, key_authorization, key_authorization_sha256, sign, JoseError};
use base64::prelude::*;
...
impl Account {
        ...
        I: IntoIterator<Item = &'a S>,
    {
        ...
        Self::create_with_keypair(client_config, directory, contact, &key_pair, None).await
    }
        ...
        I: IntoIterator<Item = &'a S>,
    {
        ...
        if let Some(external_account) = external_account {
            payload["externalAccountBinding"] =
                external_account_binding(&key_pair, &external_account.key_id, &external_account.mac_key, &directory.new_account)?;
        }
    ...
    pub fn dns_01<'a>(&self, challenges: &'a [Challenge]) -> Result<(&'a Challenge, String), AcmeError> {
        ...
        let key_auth = key_authorization_sha256(&self.key_pair, &challenge.token)?;
        Ok((challenge, BASE64_URL_SAFE_NO_PAD.encode(key_auth.as_ref())))
    }
}
