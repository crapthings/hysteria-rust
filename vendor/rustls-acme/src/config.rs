...
pub trait Dns01Solver: Send + Sync {
    /// Create the requested `_acme-challenge` TXT record and wait until it is usable.
    async fn present(&self, domain: &str, value: &str) -> Result<(), String>;

    /// Remove the TXT record created by [`Self::present`].
    async fn cleanup(&self, domain: &str, value: &str) -> Result<(), String>;
}
...
pub struct AcmeConfig<EC: Debug, EA: Debug = EC> {
    ...
    pub(crate) dns_solver: Option<Arc<dyn Dns01Solver>>,
}

/// External Account Binding credentials as defined by RFC 8555 section 7.3.4.
#[derive(Clone)]
...
pub enum UseChallenge {
    ...
    TlsAlpn01,
    Dns01,
    /// Enable both listener-based solvers and retry a failed TLS-ALPN-01
    /// authorization with HTTP-01 on a fresh order.
    Http01AndTlsAlpn01,
}
...
impl<EC: 'static + Debug, EA: 'static + Debug> AcmeConfig<EC, EA> {
    ...
    /// Configure RFC 8555 External Account Binding for account creation.
    pub fn external_account(mut self, key_id: impl Into<String>, mac_key: impl Into<Vec<u8>>) -> Self {
        self.external_account = Some(ExternalAccount {
            key_id: key_id.into(),
    ...
    /// Select DNS-01 validation and provide the record lifecycle implementation.
    pub fn dns01_solver(mut self, solver: Arc<dyn Dns01Solver>) -> Self {
        self.challenge_type = UseChallenge::Dns01;
        self.dns_solver = Some(solver);
        self
    }
