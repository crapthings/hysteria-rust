...
pub enum OrderError {
    ...
    #[error("dns-01 solver failed: {0}")]
    Dns(String),
}

impl OrderError {
    fn retryable_challenge_failure(&self) -> bool {
        let Self::BadAuth(auth) = self else {
            return false;
        };
        auth.challenges.iter().any(|challenge| {
            challenge
                .error
                .as_ref()
                .and_then(|problem| problem.typ.as_deref())
                .and_then(|kind| kind.rsplit(':').next())
                .is_some_and(|kind| matches!(kind, "connection" | "dns" | "serverInternal" | "tls" | "unauthorized"))
        })
    }
}

fn challenge_attempts(challenge_type: UseChallenge) -> &'static [UseChallenge] {
    match challenge_type {
        UseChallenge::Http01AndTlsAlpn01 => &[UseChallenge::TlsAlpn01, UseChallenge::Http01],
        UseChallenge::Http01 => &[UseChallenge::Http01],
        UseChallenge::TlsAlpn01 => &[UseChallenge::TlsAlpn01],
        UseChallenge::Dns01 => &[UseChallenge::Dns01],
    }
}
...
impl<EC: 'static + Debug, EA: 'static + Debug> AcmeState<EC, EA> {
    ...
    async fn order(config: Arc<AcmeConfig<EC, EA>>, resolver: Arc<ResolvesServerCertAcme>, account_key: Vec<u8>) -> Result<Vec<u8>, OrderError> {
        ...
        let account = Account::create_with_keypair(
            ...
            &config.contact,
            &account_key,
            config.external_account.as_ref(),
        )
        .await?;
        ...
        let challenge_attempts = challenge_attempts(config.challenge_type);
        let mut last_error = None;
        for (index, challenge_type) in challenge_attempts.iter().copied().enumerate() {
            match Self::order_attempt(&config, &resolver, &account, &key_pair, csr.der(), challenge_type).await {
                Ok(pem) => return Ok(pem),
                Err(error) if index + 1 < challenge_attempts.len() && error.retryable_challenge_failure() => {
                    resolver.clear_challenge_data();
                    log::warn!("ACME {challenge_type:?} challenge failed; retrying with the alternate challenge: {error}");
        ...
        Err(last_error.expect("challenge attempt list is never empty"))
    }

    async fn order_attempt(
        config: &AcmeConfig<EC, EA>,
        resolver: &ResolvesServerCertAcme,
        account: &Account,
        key_pair: &KeyPair,
        csr: &[u8],
        challenge_type: UseChallenge,
            ...
            match order.status {
                OrderStatus::Pending => {
                    ...
                    for url in order.authorizations.iter() {
                        Self::authorize(config, resolver, account, url, challenge_type).await?
                    }
    ...
    async fn authorize(
        ...
        let (domain, challenge_url, dns_value) = match auth.status {
            AuthStatus::Pending => {
                let Identifier::Dns(domain) = auth.identifier;
                log::info!("trigger challenge for {domain}");
                let (challenge, dns_value) = match challenge_type {
                    UseChallenge::Http01 => {
                    ...
                    UseChallenge::Dns01 => {
                        let (challenge, value) = account.dns_01(&auth.challenges)?;
                        let solver = config
                            .dns_solver
                            .as_ref()
                            .ok_or_else(|| OrderError::Dns("DNS solver is not configured".to_owned()))?;
                        solver.present(&domain, &value).await.map_err(OrderError::Dns)?;
                        (challenge, Some(value))
                    }
                    UseChallenge::Http01AndTlsAlpn01 => unreachable!("legacy mode is expanded into concrete attempts"),
                };
                if let Err(error) = account.challenge(&config.client_config, &challenge.url).await {
                    if let (Some(value), Some(solver)) = (&dns_value, &config.dns_solver) {
                        if let Err(cleanup_error) = solver.cleanup(&domain, value).await {
                            log::warn!("dns-01 cleanup failed for {domain}: {cleanup_error}");
                        }
                    }
                    resolver.clear_challenge_data();
                    return Err(error.into());
                }
                (domain, challenge.url.clone(), dns_value)
            }
            AuthStatus::Valid => {
                resolver.clear_challenge_data();
                return Ok(());
        ...
        let result = async {
            ...
            Err(OrderError::TooManyAttemptsAuth(domain.clone()))
        }
        .await;
        resolver.clear_challenge_data();
        if let Some(value) = dns_value {
            if let Some(solver) = &config.dns_solver {
                if let Err(error) = solver.cleanup(&domain, &value).await {
                    log::warn!("dns-01 cleanup failed for {domain}: {error}");
                }
            }
        }
        result
...
mod tests {
    ...
    #[test]
    fn legacy_challenges_retry_tls_then_http() {
        assert_eq!(
            challenge_attempts(UseChallenge::Http01AndTlsAlpn01),
            &[UseChallenge::TlsAlpn01, UseChallenge::Http01]
        );
        assert_eq!(challenge_attempts(UseChallenge::Dns01), &[UseChallenge::Dns01]);
    }

    #[test]
    fn retries_only_recoverable_acme_challenge_problems() {
        let auth = |kind: &str| {
            serde_json::from_value(serde_json::json!({
                ...
                "challenges": [{
                    "type": "tls-alpn-01",
                    "url": "https://ca.invalid/challenge/1",
                    "token": "token",
                    "error": { "type": format!("urn:ietf:params:acme:error:{kind}"), "detail": "failed" }
                }]
            }))
            .unwrap()
        };
        for kind in ["connection", "dns", "serverInternal", "tls", "unauthorized"] {
            assert!(OrderError::BadAuth(auth(kind)).retryable_challenge_failure());
        }
        assert!(!OrderError::BadAuth(auth("rateLimited")).retryable_challenge_failure());
        assert!(!OrderError::TooManyAttemptsAuth("example.com".to_owned()).retryable_challenge_failure());
    }
}
