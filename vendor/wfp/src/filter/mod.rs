...
use windows_sys::Win32::NetworkManagement::WindowsFilteringPlatform::{
    ...
    FWPM_FILTER_FLAG_CLEAR_ACTION_RIGHT, FWPM_FILTER_FLAG_PERSISTENT, FWPM_FILTER0, FwpmFilterAdd0,
...
impl<Name, Action> FilterBuilder<Name, Action> {
    ...
    /// Prevents lower-priority filters from overriding this filter's action.
    ///
    /// This is primarily useful for high-priority permit filters that protect
    /// an application's own control-plane traffic from broader block rules.
    pub fn clear_action_right(mut self) -> FilterBuilder<Name, Action> {
        self.filter.flags |= FWPM_FILTER_FLAG_CLEAR_ACTION_RIGHT;
        self
    }
