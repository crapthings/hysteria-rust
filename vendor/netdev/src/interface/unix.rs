...
fn sockaddr_to_network_addr(sa: *mut libc::sockaddr) -> (Option<MacAddr>, Option<IpAddr>) {
    ...
    unsafe {
        if sa.is_null() {
            ...
            let nlen = (*sa).sa_data[3] as u8 as usize;
            let alen = (*sa).sa_data[4] as u8 as usize;
            if alen > 0 && alen + nlen + 8 <= usize::from((*sa).sa_len) {
                let ptr = (*sa).sa_data.as_mut_ptr();
                let extended = std::slice::from_raw_parts_mut(ptr, 6 + nlen + alen);

                let mac = MacAddr(
                    extended[6 + nlen] as u8,
                    extended[6 + nlen + 1] as u8,
                    extended[6 + nlen + 2] as u8,
                    extended[6 + nlen + 3] as u8,
                    extended[6 + nlen + 4] as u8,
                    extended[6 + nlen + 5] as u8,
                );
