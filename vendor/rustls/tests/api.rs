...
#[cfg(feature = "aws_lc_rs")]
#[test]
fn test_server_ech_accepts_client_ech() {
    let provider = provider::default_provider();
    let hpke_suite = ALL_SUPPORTED_SUITES[0];
    let (server_key, config_list) =
        generate_server_ech_config(hpke_suite, 0x42, "public.example.com");

    let mut server_config = make_server_config(KeyType::EcdsaP256, &provider);
    server_config.ech_keys = Arc::new(rustls::server::FixedEchKeys::new(vec![server_key]));

    let ech = EchConfig::new(EchConfigListBytes::from(config_list), &[hpke_suite]).unwrap();
    let client_builder = ClientConfig::builder_with_provider(provider.into())
        .with_ech(EchMode::Enable(ech))
        .unwrap();
    let client_config = finish_client_config(KeyType::EcdsaP256, client_builder);
...
#[cfg(feature = "aws_lc_rs")]
#[test]
fn test_server_ech_rejects_mismatched_key() {
    let provider = provider::default_provider();
    let hpke_suite = ALL_SUPPORTED_SUITES[0];
    let (_, client_config_list) = generate_server_ech_config(hpke_suite, 1, "public.example.com");
    let (server_key, _) = generate_server_ech_config(hpke_suite, 2, "public.example.com");

    let mut server_config = make_server_config(KeyType::EcdsaP256, &provider);
    server_config.ech_keys = Arc::new(rustls::server::FixedEchKeys::new(vec![server_key]));
    let ech = EchConfig::new(EchConfigListBytes::from(client_config_list), &[hpke_suite]).unwrap();
    let client_builder = ClientConfig::builder_with_provider(provider.into())
        .with_ech(EchMode::Enable(ech))
        .unwrap();
    let client_config = finish_client_config(KeyType::EcdsaP256, client_builder);
