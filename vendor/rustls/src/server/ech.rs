...
use crate::log::debug;
use crate::msgs::RawClientHello;
use crate::msgs::base::Payload;
use crate::msgs::codec::{Codec, Reader};
use crate::msgs::enums::ExtensionType;
use crate::msgs::handshake::{
    ClientHelloPayload, EchConfigContents, EchConfigPayload, EncryptedClientHello,
    EncryptedClientHelloOuter, HandshakeMessagePayload, HandshakePayload, HpkeSymmetricCipherSuite,
    Random, ServerNamePayload,
};
...
impl EchServerKey {
    ...
    pub fn from_raw(
        ...
        let mut reader = Reader::init(config_bytes);
...
impl EchServerState {
    ...
    pub(crate) fn resolve<'m>(
        ...
        match &outer_hello.encrypted_client_hello {
            ...
            Some(EncryptedClientHello::Outer(_)) => {
                ...
                let outer = OuterClientHello::from_message(input)?;
                match EchFrontend::decrypt(&outer, self.keys.index())? {
                    Some((frontend, decrypted)) => {
                        let (inner_input, inner_random) = decrypted.into_message(input);
    ...
    fn resolve_retry(&mut self, input: &Message<'_>) -> Result<Option<Message<'static>>, Error> {
    ...
    fn decrypt(
        ...
        match outer.decrypt_ech(index) {
            EchDecryptResult::Accepted(decrypted, opener) => {
                ...
                let outer_server_name = match outer.hello.server_name.as_ref() {
                    Some(ServerNamePayload::SingleDnsName(name)) => Some(name.to_lowercase_owned()),
                    _ => None,
                };
                Ok(Some((
                    Self {
...
impl<'a> OuterClientHello<'a> {
    fn from_message(input: &'a Message<'a>) -> Result<Self, Error> {
        ...
        let encoded = match &input.payload {
    ...
    fn decrypt_ech(&self, index: &EchKeyIndex) -> EchDecryptResult {
        ...
        for entry in candidates {
            ...
            let Ok(encoded_inner) = opener.open(&aad, &ech_ext.payload.0) else {
                continue;
            };
    ...
    fn decode_inner_hello(&self, encoded_inner: Vec<u8>) -> Result<DecryptedEch, Error> {
        ...
        let mut reader = Reader::init(&inner_hello_encoded[4..]);
    ...
    fn compute_aad(&self, ech_ext: &EncryptedClientHelloOuter) -> Result<Vec<u8>, Error> {
        ...
        let mut aad = self
            ...
            .to_vec();
        let payload_len = ech_ext.payload.0.len();
    ...
    fn expand_outer_extensions(&self, ext_data: &[u8], out: &mut Vec<u8>) -> Result<(), Error> {
        ...
        let mut list_reader = Reader::init(ext_data);
        ...
        let mut type_reader = Reader::init(list_data);
...
impl DecryptedEch {
    /// Consume `self` and build a [`Message`] wrapping the decrypted inner
    /// ClientHello, returning the inner random alongside it.
    fn into_message(self, outer_input: &Message<'_>) -> (Message<'static>, Random) {
        let random = self.inner_hello.random;
        let input = Message {
            version: outer_input.version,
            payload: MessagePayload::Handshake {
                encoded: Payload::Owned(self.inner_hello_encoded),
                parsed: HandshakeMessagePayload(HandshakePayload::ClientHello(self.inner_hello)),
            },
        };
        (input, random)
    }
...
mod tests {
    ...
    use crate::msgs::handshake::{HpkeKeyConfig, SessionId};
    ...
    fn make_v18_contents(config_id: u8) -> EchConfigContents {
        ...
        EchConfigContents {
            key_config: HpkeKeyConfig {
                ...
                public_key: PayloadU16::new(vec![0u8; 32]),
    ...
    fn make_hello(random: [u8; 32]) -> ClientHelloPayload {
        ...
        use crate::msgs::handshake::{
            ClientExtensions, Random, SessionId, SupportedProtocolVersions,
        };
    ...
    fn inner_hello_strips_padding() {
        ...
        outer.session_id = SessionId::read(&mut Reader::init(&sid_bytes)).unwrap();
    ...
    fn rejected_no_keys() {
        ...
        hello.encrypted_client_hello =
            Some(EncryptedClientHello::Outer(EncryptedClientHelloOuter {
                cipher_suite: HpkeSymmetricCipherSuite::default(),
                config_id: 1,
                enc: PayloadU16::new(vec![0u8; 32]),
                payload: PayloadU16::new(vec![0u8; 64]),
            }));
